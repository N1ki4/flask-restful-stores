	...
	def test_create_store_already_exist(self):
		with self.app() as client:
			with self.app_context():
				...
				self.assertDictEqual({'message': 'A store test already exists.'},
	...
	def test_store_found_with_items(self):
		with self.app() as client:
			with self.app_context():
				...
				self.assertEqual(response.status_code, 200)
				self.assertDictEqual({'name': 'test', 'items': [{'name': 'test', 'price': 19.99, 'store_id': 1}]},
									   json.loads(response.data))
	...
	def test_store_list_with_items(self):
		with self.app() as client:
			with self.app_context():
				...
				self.assertDictEqual({'stores': [
					{'name': 'test1', 'items': []},
					{'name': 'test2', 'items': [{'name': 'test123', 'price': 19.99, 'store_id': 2}]},
				]}, json.loads(response.data))
