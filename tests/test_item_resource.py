        ...
        with self.app() as client:
            with self.app_context():
                ...
                response = client.post(
                    "/item/test_item",
                    data=json.dumps({"store_id": 1, "price": 88.99}),
                    headers={"Content-Type": "application/json"},
                )
                self.assertEqual(response.status_code, 201)
                self.assertIsNotNone(ItemModel.find_by_name("test_item"))
                self.assertDictEqual(
                    {"name": "test_item", "price": 88.99, "store_id": 1},
        ...
        with self.app() as client:
            with self.app_context():
                ...
                auth_request = client.post(
                    "/auth",
                    data=json.dumps({"username": "test", "password": "pass1234"}),
                    headers={"Content-Type": "application/json"},
                )
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                response = client.put(
                    "/item/test_item",
                    data=json.dumps({"price": 13.99, "store_id": 1}),
                    headers={"Content-Type": "application/json"},
                )
                ...
                self.assertDictEqual(
                    {"name": "test_item", "price": 13.99, "store_id": 1},
                    json.loads(response.data),
                )
